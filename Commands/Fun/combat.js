...
const moves = [
	{
		...
		use: (you, opponent, log, pRef) => {
			...
			if (Math.floor(Math.random() * 4) == 0) { dmg = dmg * 3, log.push(`${you.name} rolled a crit!`) };
	...
	},
	{
		...
		images: [
			...
			"https://static.wikia.nocookie.net/narutofanon/images/e/e0/Raijin-niss.jpg/revision/latest/scale-to-width-down/340?cb=20140107144112"
		],
		use: (you, opponent, log, pRef) => {
	...
	},
	{
		...
		use: (you, opponent, log, pRef) => {
			...
			var txt = pRef ? "Light Mode equips the Sword of Revealing Light. Damage massively increased! Light Mode is able to perform ShockWave attacks! Light mode stands become more effective!" : "smh that move doesn't exist";
			log.push(txt);
		}
	},
	{
		...
		use: (you, opponent, log, pRef) => {
			...
			if (pRef)
			{
				opponent.hp -= Math.round(Math.random() * 100) + 10;
				opponent.blocked += 1;
				you.hp -= 10;
			}

			var txt = pRef ? "Light Mode summons the Sun, dealing drastic electromagnetic light to both users! Huh, who knew the sun itself is a stand?" : "smh that move doesn't exist";
			log.push(txt);
		}
	},
	{
		...
		use: (you, opponent, log, pRef) => {
			...
			var txt = pRef ? "Light mode summons the Star Platinum! Time is now frozen for 3 turns for Dark Mode! Now that's enough time to hide from Duck for part skipping." : "smh that move doesn't exist";
			log.push(txt);
	...
	},
	{
		...
		use: (you, opponent, log, pRef) => {
			...
			var txt = pRef ? "Light Mode summons Dimo! It's literally a ripoff Pokemon from China but hey, at least it does its job.\nThe vortex of light blinds Dark Mode and significantly drops Dark Mode's attack!" : "smh this move does not exist";
			log.push(txt);
	...
	},
	{
		...
		use: (you, opponent, log, pRef) => {
			...
			{
				log.push("<Order 66 /> <$execute_command /> : Success");
			}
			opponent.hp -= 100;
		}
	}
	...
	execute: (message, args, toolkit, currentChannel) => {
		switch (args[0])
		{
			...
			case "dark":
				...
				let gameTrack = [{hp: 100, defense: 0, attack: 1, name: "MIT", blocked: 0}, {hp: 100, defense: 0, attack: 1, name: "Harvard :pepega:", blocked: 0, tdf: 0, tof: 0}];
				...
				message.channel.send(combatEmbed(gameTrack, "N/A", img))
					.then(msgObj => {
						...
						function playMove(messageContent, message)
						{
							var gameEnd = false;
							var botPhoto;
							var attacks = moves.filter(x => x.name.toUpperCase() == messageContent.toUpperCase() && (x.faction == "Neutral" || player.name.includes(x.faction)));
							if (attacks.length)
							{
								var log = [""];
								if (player.blocked)
								...
								if (Math.floor(Math.random() * 3) == 1 || player.blocked)
								{
									msgObj.edit(combatEmbed(gameTrack, log.join("\n➡  "), botPhoto, isLight));
								}
								else
								{
									msgObj.edit(combatEmbed(gameTrack, log.join("\n➡  "), Helpy.randomResp(attacks[0].images), isLight));
								}
...
function lightModeAI(playerObj, botObj, log)
{
	...
	else if (playerObj.hp < 25 || Math.floor(Math.random() * 10) == 0)
	{
